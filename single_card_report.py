...
def generate_excel(items, avg_prices, today):
    ...
    c = ws["A2"]
    c.value = f"生成时间: {now_str} | 汇率: 1 JPY = {rate} CNY | 数据范围: 最近7天 | 低价预警: 价格 < 均价50%"
...
def main():
    ...
    out_path = os.path.join(OUTPUT_DIR, f"监控_单卡_{today}.xlsx")
    gfr.generate_final_report(json_path, out_path)
    graded_cnt = len([i for i in unique if i.get('type') == 'graded'])
    raw_cnt = len([i for i in unique if i.get('type') == 'raw'])
    warning_cnt = len([i for i in unique if i.get('status') == '販売中'])
